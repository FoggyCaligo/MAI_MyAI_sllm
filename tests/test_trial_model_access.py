...
def _principal(role: AccessRole) -> AccessPrincipal:
    return AccessPrincipal(
        auth_user_id=role.value,
        memory_user_id=role.value,
        role=role,
    )
...
def test_trial_uses_runtime_default_when_model_is_omitted_or_matches() -> None:
    ...
    assert _selected_model_for_principal(
        trial,
        runtime_model="ornith-1.5:9b",
...
def test_owner_keeps_full_model_selection() -> None:
    ...
    assert _visible_models_for_principal(
        owner,
        runtime_model="ornith-1.5:9b",
