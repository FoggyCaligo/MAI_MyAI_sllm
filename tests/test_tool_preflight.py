...
def test_preflight_freezes_model_selected_known_tools() -> None:
    ...
    adapter = FakeAdapter('{"terminal_run":true}')
    ...
    assert request.response_format["required"] == ["terminal_run"]
    assert request.response_format["properties"]["terminal_run"] == {
        "type": "boolean",
        "description": "Run a command in the local terminal.",
    }
...
def test_preflight_unknown_tool_fails_explicitly() -> None:
    ...
    planner = OllamaToolRequirementPlanner(
        FakeAdapter('{"terminal_run":false,"imaginary_tool":true}')
    )
...
def test_preflight_rejects_extra_structured_fields() -> None:
    planner = OllamaToolRequirementPlanner(FakeAdapter('{"unexpected":true}'))
