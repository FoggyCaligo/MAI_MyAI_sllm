...
class AccessPolicy:
    ...
    @classmethod
    def from_env_values(cls, *, owner_id: str | None, trial_ids: str | None) -> "AccessPolicy":
        if owner_id is None:
        ...
        return cls(owner_id=owner_id, trial_ids=frozenset(trials))
    ...
    def authenticate(self, submitted_id: str) -> AccessPrincipal:
        ...
        if user_id == self.owner_id:
            return AccessPrincipal(user_id=user_id, role=AccessRole.OWNER)
        if user_id in self.trial_ids:
            return AccessPrincipal(user_id=user_id, role=AccessRole.TRIAL)
