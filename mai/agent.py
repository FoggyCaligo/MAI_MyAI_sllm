...
from .memory_completion import GraphMemoryCommitLoop
        ...
        *,
        ...
        try:
            ...
            with phase(resolved_turn_id, "agent"):
                fixed_answer, _, work_events = self._run_agent_phase(
                    context=WorkContext(
                        user_id=user_id,
                        turn_id=resolved_turn_id,
            ...
            with phase(resolved_turn_id, "memory_mutation"):
                memory_result = GraphMemoryCommitLoop(
                    model=self.model,
                    executor=self.memory_executor,
                ).run(
                    user_id=user_id,
                    turn_id=resolved_turn_id,
                    user_text=clean_user,
            ...
            if memory_result.get("status") != "done":
                raise RuntimeError("graph memory commit did not complete")
        ...
        messages: list[dict[str, str]] = [
            {
                ...
                "content": (
                    ...
                    f"{catalog}. Call tool_manual for a tool before using that work tool. The final answer contains only "
                    "the conversational outcome and content. After that answer is frozen, the framework runs a separate "
                    "graph-memory commit loop, so do not plan graph mutations in the answer."
                ),
            },
        ...
        while True:
            ...
            if action.get("action") == "answer":
                ...
                if outcome not in {"completed", "blocked"}:
                    raise ModelContractError("answer outcome must be completed or blocked")
