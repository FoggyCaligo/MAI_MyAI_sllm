...
from ..memory.vector.embedding import OllamaEmbeddingProvider
from ..memory.vector.sqlite_vec import SqliteVecIndex
...
def build_experiment(
    *,
    ...
    if not embedding_model.strip():
        raise ValueError("embedding_model must be explicitly configured for the C experiment")

    graph = MemoryGraphRepository(memory_db_path)
    segmenter = SentenceBreakerSegmenter(db_path=str(sentence_breaker_db_path))
    embedding = OllamaEmbeddingProvider(embedding_model, host=ollama_host)
    vector = SqliteVecIndex(memory_db_path, embedding)
    ...
    return PureAgentExperiment(user_id, agent, memory, graph, vector, segmenter)
...
def _parse_args() -> argparse.Namespace:
    ...
    if not args.embedding_model:
        parser.error("--embedding-model or EMBEDDING_MODEL is required; the experiment does not choose one implicitly")
    return args
