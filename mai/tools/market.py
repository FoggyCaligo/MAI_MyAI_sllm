...
HTTP failures and incompatible response shapes therefore fail visibly.
"""
...
class MarketDataInput(BaseModel):
    ...
    query: str = Field(min_length=1, description="Korean listed company name or stock code to resolve through Naver Finance")
...
def _resolve_stock(client: httpx.Client, query: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    payload = _request_json(client, _AUTOCOMPLETE_URL, params={"query": query, "target": "stock"})
...
def market_data(query: str) -> dict[str, object]:
    ...
    return {
        ...
        "query": clean_query,
        "resolution": {"selected": selected, "matches": matches, "selection_basis": "naver_autocomplete_rank_1"},
        "stock": {"code": code, "name": basic.get("stockName") or integration.get("stockName") or selected.get("name"), "market": market},
        "quote": {
        ...
        "range_52w": {"high": _metric(metrics, "highPriceOf52Weeks"), "low": _metric(metrics, "lowPriceOf52Weeks")},
        "foreign_rate": _metric(metrics, "foreignRate"),
        "source": {"basic": _BASIC_URL.format(code=code), "integration": _INTEGRATION_URL.format(code=code)},
    }


def register_market_tools(registry: ToolRegistry, *, timeout_seconds: float | None = 30) -> None:
    registry.add(
        name="market_data",
        description=(
